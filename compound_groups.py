...
"""
This script is adapted from inchi2gv.py in https://github.com/eladnoor/component-contribution, with 17 additional group definitions 
"""
...
class GroupDecomposer(object):
    ...
    def Decompose(self, mol, ignore_protonations=False, strict=False):
        ...
        #print 'all atoms are ',unassigned_nodes
        groups = []
            ...
            # Not a phosphate group or expanded correction.
                ...
                for nodes in mol.FindSmarts(group.smarts):
                    try:
                        ...
                        #print nodes, focal_set, group.smarts
                    except IndexError:
        ...
        #print 'length of the groups are', len(groups)

        #print [[cur_group[0].name, cur_group[0].smarts, cur_group[1][0]] for cur_group in groups if cur_group[1] != []]
        #print 'left over atoms including protons ', unassigned_nodes
        ...
        #print 'left over atoms excluding protons',unassigned_nodes

        decomposition = GroupDecomposition(self.groups_data, mol,
                                           groups, unassigned_nodes)

        #comment this out if we still want the group vector from nondecomposable compounds
